...
// 5.1 surround) by driving an array of regular `Decoder`s and routing PCM
...
// instead the caller owns the `[Decoder]` storage and passes it in. The
// `MsDecoder` itself is just the channel layout.
//
...
impl MsDecoder {
    ...
    /// [`nb_streams`](MsDecoder::nb_streams) decoders, each created with the
    /// matching [`stream_channels`](MsDecoder::stream_channels). The output
    ...
    pub fn final_range(&self, decoders: &[Decoder]) -> u32 {
...
mod tests {
    use super::*;
    use crate::opus_decoder::{Channels, Decoder, SampleRate};

    #[test]
    fn new_validates_args() {
        assert!(MsDecoder::new(2, 1, 1, &[0, 1]).is_ok());
        assert_eq!(MsDecoder::new(2, 0, 0, &[0, 1]).err(), Some(Error::BadArg)); // streams < 1
        assert_eq!(MsDecoder::new(2, 1, 2, &[0, 1]).err(), Some(Error::BadArg)); // coupled > streams
        // mapping references a channel beyond the stream count
        assert_eq!(MsDecoder::new(2, 1, 0, &[0, 9]).err(), Some(Error::BadArg));
    }
