            ...
            loop {
                ...
                if budget - tell >= 15 {
                    ...
                    qi = ec_laplace_decode(
                        ...
                        ((prob_model[pi + 1] as u32) << 6) as c_int,
                    );
                ...
                *old_ebands.add(i + c * nb_ebands) = max16(
                    -qconst16(9.0, DB_SHIFT),
                    *old_ebands.add(i + c * nb_ebands),
                );
                let mut tmp: OpusVal32 = pshr32(
                    mult16_16(coef, *old_ebands.add(i + c * nb_ebands)),
                    8,
                ) + prev[c]
                    + shl32(q, 7);
                #[cfg(feature = "fixed-point")]
...
pub unsafe extern "C" fn unquant_fine_energy(
    ...
    unsafe {
        ...
        for i in (start as usize)..(end as usize) {
            ...
            let mut c: usize = 0;
            loop {
                let q2 = ec_dec_bits(dec, *fine_quant.add(i) as u32) as i32;
                let offset: OpusVal16;
                #[cfg(feature = "fixed-point")]
                {
                    offset = sub16(
                        shr32(
                            shl32(extend32(q2 as i16), DB_SHIFT)
                                + qconst16(0.5, DB_SHIFT) as i32,
                            *fine_quant.add(i),
                        ) as i16,
                ...
                #[cfg(not(feature = "fixed-point"))]
                {
                    offset = (q2 as f32 + 0.5)
                        * ((1 << (14 - *fine_quant.add(i))) as f32)
                        * (1.0 / 16384.0)
                        - 0.5;
                }
                *old_ebands.add(i + c * nb_ebands) += offset;
    ...
    unsafe {
        ...
        for prio in 0..2 {
            ...
            while i < end as usize && bits_left >= c_channels {
                if *fine_quant.add(i) >= MAX_FINE_BITS as c_int
                    || *fine_priority.add(i) != prio
                {
                    i += 1;
                    continue;
                }
                ...
                loop {
                    ...
                    {
                        offset = shr16(
                            shl16(q2 as i16, DB_SHIFT) - qconst16(0.5, DB_SHIFT),
                            *fine_quant.add(i) as i32 + 1,
                        );
                    }
                    #[cfg(not(feature = "fixed-point"))]
                    {
                        offset = (q2 as f32 - 0.5)
                            * ((1 << (14 - *fine_quant.add(i) - 1)) as f32)
                            * (1.0 / 16384.0);
                    }
                    *old_ebands.add(i + c * nb_ebands) += offset;
                    bits_left -= 1;
    ...
    unsafe {
        ...
        loop {
            for i in 0..(start as usize) {
                *e_bands.add(i + c * nb_ebands) = 0 as OpusVal32;
            }
            for i in (start as usize)..(end as usize) {
                let lg = add16(
                    *old_ebands.add(i + c * nb_ebands),
                    shl16(E_MEANS[i] as OpusVal16, 6),
                );
                *e_bands.add(i + c * nb_ebands) = pshr32(celt_exp2(lg), 4);
            }
    ...
    unsafe {
        ...
        loop {
            for i in 0..(eff_end as usize) {
                *band_log_e.add(i + c * nb_ebands) = celt_log2(shl32(
                    *band_e.add(i + c * nb_ebands),
                    2,
                )) - shl16(E_MEANS[i] as OpusVal16, 6);
            }
