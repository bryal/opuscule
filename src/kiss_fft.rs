...
pub fn ki_bfly5(fout: &mut [KissFftCpx], fstride: usize, st: &KissFftState, m: usize, n: usize, mm: usize) {
    ...
    for i in 0..n {
        ...
        for u in 0..m {
            ...
            scratch[2] = c_mulc(fout[base + u + 2 * m], st.twiddles[2 * u * fstride]);
            scratch[3] = c_mulc(fout[base + u + 3 * m], st.twiddles[3 * u * fstride]);
            scratch[4] = c_mulc(fout[base + u + 4 * m], st.twiddles[4 * u * fstride]);
