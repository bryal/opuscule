...
use super::macros::{silk_add_rshift, silk_lshift, silk_mla, silk_smlawb, silk_smulbb};
...
pub extern "C" fn silk_log2lin(in_log_q7: i32) -> i32 {
    ...
    if in_log_q7 < 2048 {
        ...
        out = silk_add_rshift(out, out * silk_smlawb(frac_q7, silk_smulbb(frac_q7, 128 - frac_q7), -174), 7);
    } else {
        /* Piece-wise parabolic approximation */
        out = silk_mla(out, out >> 7, silk_smlawb(frac_q7, silk_smulbb(frac_q7, 128 - frac_q7), -174));
    }
    out
}
