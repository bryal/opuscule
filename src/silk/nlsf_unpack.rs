...
pub unsafe extern "C" fn silk_NLSF_unpack(
    ...
    unsafe {
        ...
        while i < order {
            ...
            *ec_ix.offset(i as isize) = silk_smulbb(silk_rshift(entry as i32, 1) & 7, 2 * NLSF_QUANT_MAX_AMPLITUDE + 1) as i16;
            *pred_q8.offset(i as isize) = *(*ps_nlsf_cb).pred_q8.offset((i + (entry as i32 & 1) * (order - 1)) as isize);
            *ec_ix.offset((i + 1) as isize) =
                silk_smulbb(silk_rshift(entry as i32, 5) & 7, 2 * NLSF_QUANT_MAX_AMPLITUDE + 1) as i16;
            *pred_q8.offset((i + 1) as isize) =
                *(*ps_nlsf_cb).pred_q8.offset((i + (silk_rshift(entry as i32, 4) & 1) * (order - 1) + 1) as isize);
            i += 2;
        }
    }
}
