...
pub fn silk_decode_pitch(lag_index: i16, contour_index: i8, pitch_lags: &mut [i32], fs_khz: i32, nb_subfr: i32) {
    let (lag_cb, cbk_size): (&[i8], i32) = if fs_khz == 8 {
        if nb_subfr == PE_MAX_NB_SUBFR {
            (SILK_CB_LAGS_STAGE2.as_flattened(), PE_NB_CBKS_STAGE2_EXT)
        } else {
            (SILK_CB_LAGS_STAGE2_10_MS.as_flattened(), PE_NB_CBKS_STAGE2_10MS)
        }
    } else if nb_subfr == PE_MAX_NB_SUBFR {
        (SILK_CB_LAGS_STAGE3.as_flattened(), PE_NB_CBKS_STAGE3_MAX)
    } else {
        (SILK_CB_LAGS_STAGE3_10_MS.as_flattened(), PE_NB_CBKS_STAGE3_10MS)
    };
    ...
    for k in 0..nb_subfr {
        ...
        let cb_val = lag_cb[(k * cbk_size + contour_index as i32) as usize];
        pitch_lags[k as usize] = (lag + cb_val as i32).clamp(min_lag, max_lag);
    }
}
