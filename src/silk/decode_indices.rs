...
pub fn silk_decode_indices(
    ...
    if cond_coding == CODE_CONDITIONALLY {
        ...
        ps_dec.indices.gains_indices[0] =
            silk_lshift(ec_dec_icdf(ps_range_dec, &SILK_GAIN_ICDF[ps_dec.indices.signal_type as usize], 8), 3) as i8;
        ps_dec.indices.gains_indices[0] =
            (ps_dec.indices.gains_indices[0] as i32 + ec_dec_icdf(ps_range_dec, &SILK_UNIFORM8_ICDF, 8)) as i8;
    }

    /* Remaining subframes */
    let mut i = 1;
    while i < ps_dec.nb_subfr {
        ps_dec.indices.gains_indices[i as usize] = ec_dec_icdf(ps_range_dec, &SILK_DELTA_GAIN_ICDF, 8) as i8;
        i += 1;
    }
    ...
    /* Decode LSF Indices */
    /**********************/
    let nlsf_cb = ps_dec.ps_nlsf_cb.or_panic("NLSF codebook is set by silk_decoder_set_fs before decoding");
    ...
    while i < nlsf_cb.order as i32 {
        let mut ix = ec_dec_icdf(ps_range_dec, &nlsf_cb.ec_icdf[ec_ix[i as usize] as usize..], 8);
        if ix == 0 {
    ...
    if ps_dec.nb_subfr == MAX_NB_SUBFR as i32 {
        ps_dec.indices.nlsf_interp_coef_q2 = ec_dec_icdf(ps_range_dec, &SILK_NLSF_INTERPOLATION_FACTOR_ICDF, 8) as i8;
    } else {
    ...
    if ps_dec.indices.signal_type as i32 == TYPE_VOICED {
        ...
        if cond_coding == CODE_CONDITIONALLY && ps_dec.ec_prev_signal_type == TYPE_VOICED {
            /* Decode Delta index */
            let mut delta_lag_index = ec_dec_icdf(ps_range_dec, &SILK_PITCH_DELTA_ICDF, 8) as i16;
        ...
        if decode_absolute_lag_index != 0 {
            ...
            ps_dec.indices.lag_index += ec_dec_icdf(
                ps_range_dec,
                ps_dec.pitch_lag_low_bits_icdf.or_panic("pitch ICDF is set by silk_decoder_set_fs"),
                8,
            ) as i16;
        }
        ...
        ps_dec.indices.contour_index =
            ec_dec_icdf(ps_range_dec, ps_dec.pitch_contour_icdf.or_panic("pitch ICDF is set by silk_decoder_set_fs"), 8) as i8;

        /********************/
        /* Decode LTP gains */
        /********************/
        /* Decode PERIndex value */
        ps_dec.indices.per_index = ec_dec_icdf(ps_range_dec, &SILK_LTP_PER_INDEX_ICDF, 8) as i8;

        let mut k = 0;
        while k < ps_dec.nb_subfr {
            ps_dec.indices.ltp_index[k as usize] =
                ec_dec_icdf(ps_range_dec, SILK_LTP_GAIN_ICDF_PTRS[ps_dec.indices.per_index as usize], 8) as i8;
            k += 1;
        }
        ...
        if cond_coding == CODE_INDEPENDENTLY {
            ps_dec.indices.ltp_scale_index = ec_dec_icdf(ps_range_dec, &SILK_LTP_SCALE_ICDF, 8) as i8;
        } else {
