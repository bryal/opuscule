...
pub fn silk_decode_parameters(ps_dec: &mut SilkDecoderState, ps_dec_ctrl: &mut SilkDecoderControl, cond_coding: i32) {
    ...
    /* Decode NLSFs */
    /****************/
    silk_nlsf_decode(&mut p_nlsf_q15, &ps_dec.indices.nlsf_indices, ps_dec.ps_nlsf_cb.unwrap());
    ...
    if (ps_dec.indices.nlsf_interp_coef_q2 as i32) < 4 {
        ...
        let (first, second) = ps_dec_ctrl.pred_coef_q12.split_at_mut(1);
        first[0][..lpc_order].copy_from_slice(&second[0][..lpc_order]);
    }
    ...
    if ps_dec.indices.signal_type as i32 == TYPE_VOICED {
        ...
        let cbk = SILK_LTP_VQ_PTRS_Q7[ps_dec.indices.per_index as usize];

        let mut k = 0i32;
        while k < ps_dec.nb_subfr {
            let ix = ps_dec.indices.ltp_index[k as usize] as usize;
            let mut i = 0;
            while i < LTP_ORDER {
                ps_dec_ctrl.ltp_coef_q14[k as usize * LTP_ORDER + i] = silk_lshift(cbk[ix][i] as i32, 7) as i16;
                i += 1;
            }
            k += 1;
        }
        ...
        let n = ps_dec.nb_subfr as usize;
        ps_dec_ctrl.pitch_l[..n].fill(0);
        ps_dec_ctrl.ltp_coef_q14[..LTP_ORDER * n].fill(0);
