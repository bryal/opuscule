...
pub fn silk_sum_sqr_shift(energy: &mut i32, shift: &mut i32, x: &[i16]) {
    let mut nrg: i32 = 0;
    let mut shft: i32 = 0;
    let pair_end = x.len() as i32 - 1;

    let mut i = 0;
    while i < pair_end {
        nrg = silk_smlabb_ovflw(nrg, x[i as usize] as i32, x[i as usize] as i32);
        nrg = silk_smlabb_ovflw(nrg, x[(i + 1) as usize] as i32, x[(i + 1) as usize] as i32);
        if nrg < 0 {
            /* Scale down */
    ...
    while i < pair_end {
        let mut nrg_tmp = silk_smulbb(x[i as usize] as i32, x[i as usize] as i32);
        nrg_tmp = silk_smlabb_ovflw(nrg_tmp, x[(i + 1) as usize] as i32, x[(i + 1) as usize] as i32);
    ...
    if i == pair_end {
        ...
        let nrg_tmp = silk_smulbb(x[i as usize] as i32, x[i as usize] as i32);
        nrg = (nrg as u32 + (nrg_tmp as u32 >> shft)) as i32;
    }
