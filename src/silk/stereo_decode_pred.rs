...
pub fn silk_stereo_decode_pred(ps_range_dec: &mut ec_dec, pred_q13: &mut [i32; 2]) {
    ...
    ix[1][2] = n - 5 * ix[0][2];
    for n in 0..2 {
        ix[n][0] = ec_dec_icdf(ps_range_dec, &SILK_UNIFORM3_ICDF, 8);
        ix[n][1] = ec_dec_icdf(ps_range_dec, &SILK_UNIFORM5_ICDF, 8);
    }
...
pub fn silk_stereo_decode_mid_only(ps_range_dec: &mut ec_dec, decode_only_mid: &mut i32) {
    *decode_only_mid = ec_dec_icdf(ps_range_dec, &SILK_STEREO_ONLY_CODE_MID_ICDF, 8);
}
