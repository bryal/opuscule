...
// Source file lists below are copied verbatim from the .mk files in c/.
...
fn main() {
    ...
    if fixed {
        for f in SILK_SOURCES_FIXED {
            build.file(format!("c/{f}"));
        }
    } else {
        for f in SILK_SOURCES_FLOAT {
            build.file(format!("c/{f}"));
        }
    }
